...
public class Zipper {
  ...
  public String encode(String plainText) {
	...
	for(int i = 0; i < length; i++) {
		char tempCharacter = plainText.charAt(i);
		sb.append(book.encodeChar(tempCharacter));
	}
  ...
  public String decode(String bits) {
	...
	boolean done = false;
	while(!done) {
		sb.append(ht.decodeChar(bits));
		char testCharacter = ht.decodeChar(bits);
		int length = book.encodeChar(testCharacter).length();
